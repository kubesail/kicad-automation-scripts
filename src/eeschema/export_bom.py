...
def eeschema_export_bom(output_dir, eeschema_proc):
    ...
    #TODO copy hotkey file (~/.config/kicad/5.99/user.hotkeys)

    logger.info('Open Bill Of Materials window')
    xdotool(['key', 'alt+shift+b'])
    ...
    logger.info('Paste JLCPCB BOM command')
    logger.info("Select commandline window");
    xdotool(['key', 'Tab', 'Tab', 'Tab', 'Tab', 'Tab', 'Tab']);
    jlcpcb_bom_file = output_dir + "jlc_bom.csv"
    clipboard_store('/usr/bin/python3 "/usr/share/kicad-nightly/plugins/jlcpcb_bom.py" "%I" "'+jlcpcb_bom_file+'"');
    xdotool(['key', 'ctrl+v'])
    xdotool(['key', 'Return'])
    logger.info("Waiting for JLCPCB BOM file")
    # this might take a moment. Wait for the bom.csv to appear
    file_util.wait_for_file_created_by_process(eeschema_proc.pid, jlcpcb_bom_file)
    # Seems to be a bit of a delay here
    sleep(2)
